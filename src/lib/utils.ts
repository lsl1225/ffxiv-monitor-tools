...
export const doesJobNeedMPBar = (job: Job): boolean =>
  Util.isCasterDpsJob(job) ||
  Util.isHealerJob(job) ||
  kMeleeWithMpJobs.includes(job)
...
export const calcGCDFromStat = (
  ...
  if (player.job === 'BLM') {
    ...
    if (player.speedBuffs.fuka) {
      if (player.level > 77) type1Buffs += 13
      else type1Buffs += 10
    }
  }
  ...
  const gcdMs =
    (Math.floor(1000 - Math.floor((130 * (stat - mod[0])) / mod[1])) *
      actionDelay) /
    1000
  const a = (100 - type1Buffs) / 100
  const b = (100 - type2Buffs) / 100
  const gcdC = Math.floor(
    (Math.floor((a * b * gcdMs) / 10) * astralUmbralMod) / 100
  )
  return gcdC / 100
}

export const computeBackgroundColorFrom = (
  element: HTMLElement,
  classList: string
): string => {
