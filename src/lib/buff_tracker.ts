...
export class Buff {
  ...
  addCooldown(source: string, effectSeconds: number): void {
    ...
    let secondsUntilShow =
      this.info.cooldown - this.options.BigBuffShowCooldownSeconds
    secondsUntilShow = Math.min(
      Math.max(effectSeconds, secondsUntilShow, 1),
      this.info.cooldown
    )
  ...
  bigBuffAutoWidth(seconds: number): number {
    ...
    let width =
      ...
      (this.options.DotIconWidth + this.options.DotBorderSize) -
      5
    let c =
      width > this.options.BigBuffBarMaxWidth
        ? this.options.BigBuffBarMaxWidth / 30
        : width / 30
    return seconds * c
  }
...
export class BuffTracker {
    ...
    for (const [key, buffOmitName] of Object.entries(this.buffInfo)) {
      ...
      for (const propStr in propToMapMap) {
        ...
        if (typeof key === 'undefined') {
          console.error(
            'undefined value for key ' + prop + ' for buff ' + buff.name
          )
          continue
        }
  ...
  onUseAbility(id: string, matches: Partial<NetMatches['Ability']>): void {
    ...
    for (const b of buffs) {
      ...
      let seconds = -1
      if (b.useEffectDuration) seconds = parseFloat(matches?.duration ?? '0')
      else if ('durationSeconds' in b) seconds = b.durationSeconds ?? seconds
  ...
  onGainEffect(
    ...
    for (const b of buffs) {
      ...
      if (matches.count != null && matches.count !== '00') {
        if (
          b.physicalUpCount != null &&
          b.physicalUpCount[matches.count] != null
        ) {
        ...
        let dom = findCountBuff(
          this.buffsListDiv,
          matches?.targetId + '=>' + b.name
        )
        if (dom !== null) {
      ...
      if (
        (b.meleeUp != null && b.meleeUp > 0) ||
        (b.rangedUp != null && b.rangedUp > 0)
      ) {
        if (
          Util.isMeleeDpsJob(this.playerJob) ||
          Util.isTankJob(this.playerJob)
        ) {
  ...
  onLoseEffect(
    ...
    if (!buffs) return
    for (const b of buffs) this.onLoseBigBuff(matches?.targetId, b.name)
  }
  ...
  onYouLoseEffect(
    ...
    this.onLoseEffect(
      this.loseEffectMap[name] ?? this.cooldownAbilityMap[name],
      matches
    )
  }
