...
import { kComboActions, kComboBreakers, kComboDelay } from './constants'
...
export class ComboTracker extends EventEmitter<{ combo: ComboCallback }> {
  ...
  StateTransition(id?: string, nextState?: StartMap[string]): void {
    ...
    this.isFinalSkill =
      (nextState && Object.keys(nextState.next).length === 0) ?? false
  ...
  static setup(player: Player): ComboTracker {
    let breakers
